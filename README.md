...
- `Jump.CredoChecks.AssertReceiveTimeout`: Flags `assert_receive` calls that specify explicit timeouts. These timeouts can seem like a fine idea in dev, but when your test later runs on a dog-slow CI machine, the tight timeouts can cause flakiness. (In CI, you might run with an absurdly long timeout to avoid flakiness.)
    ...
    - Also supports an optional `max_refute_receive_timeout` parameter that flags `refute_receive` calls whose timeout exceeds the configured maximum (because `refute_receive` always blocks for its full timeout, setting a lower bound on the test's runtime). The same cap applies to PhoenixTest `refute_has` calls that pass an explicit `:timeout`.
...
- `Jump.CredoChecks.UseObanProWorker`: Ensures your Oban worker modules consistently `use Oban.Pro.Worker` rather than `use Oban.Worker` so that you get all the benefits of the Pro package.
    - Supports an optional `:supported_worker_modules` parameter (default: `[Oban.Pro.Worker]`). Replace or extend the list with project-specific wrappers.
    - Also flags `process/1` callbacks that expect string keys in the job args when the worker declares an `args_schema`, since the schema guarantees the args always have atom keys and such matches can never succeed.
    ...
    # ❌ Never matches — args_schema casts args to atom keys
...
1. Add `jump_credo_checks` to your `mix.exs` dependencies:
    ...
    def deps do
      [
        {:jump_credo_checks, "~> 0.5", only: [:dev], runtime: false},
      ]
    end
    ```
        ...
        %{
          checks: %{
            enabled: [
              ...
              # (any explicit assert_receive timeout is flagged; refute_receive/refute_has timeouts are not checked).
              ...
              #   the configured value, and PhoenixTest refute_has calls that pass an explicit timeout
              #   longer than the configured value. Because these refutes wait out their timeout,
              #   a long one sets a lower bound on how long the entire test takes to run.
              {Jump.CredoChecks.AssertReceiveTimeout,
              ...
              # Default supported_worker_modules is [Oban.Pro.Worker].
              {Jump.CredoChecks.UseObanProWorker,
               supported_worker_modules: [Oban.Pro.Worker, MyApp.CustomObanWorker]},
